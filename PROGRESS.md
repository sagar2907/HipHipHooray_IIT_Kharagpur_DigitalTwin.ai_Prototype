...
| **08-28 17:10** | **Complexity 6 CLOSED** — transfer + sensor maturity measured; regret transfers, top-1 does not | Sagar | `done` |
...
| **08-29 01:10** | **Complexity 1 CLOSED** — checklist honesty test; **coverage 6/7** | Sagar | `done` |
| **08-29 03:30** | **Dashboard readability rebuild** — 16/16 WCAG AA (was 4/9); 12px type floor | Sagar | `done` |
| **08-29 02:20** | **Fresh-clone check caught a submission blocker** — demo was unrunnable outside this machine; sample shift now shipped | Sagar | `done` |
| **08-29 01:45** | **Documents synced to current state** — checklist + coverage findings folded in, 2 stale rows caught and fixed | Sagar | `done` |
...
|---|---|---|---|
| 1 | Inconsistent sensor coverage; some stations on **manual checklists** | ✅ | **CLOSED 08-29.** Dark stations: **15.5% of forming warnings name a station with no sensors** (n=29,060). Checklists: **96.51% pass rate but 2.76% escape rate** on 31,329 entries — it measures compliance, not quality (`results/manual_check_eval.csv`). Coverage **75.3% instrumented / 24.7% manual** across body+final — exact PS match |
...
| 3 | PLC risk; retrofits only in **maintenance windows** | ✅ | **CLOSED 08-28.** Advisory-only boundary on screen *and* a **window-dated retrofit schedule** ranked by exposure closed per rupee, with the cost of deferring stated (~Rs 101,560 / 6 months). `results/sensor_schedule.csv` |
...
| 6 | Scaling: **layout, equipment vintage, sensor maturity** | ✅ | **CLOSED 08-28.** Layout transfer over 4 topologies: regret stable 1.21–1.43 while top-1 collapses 44.5%→10.6%. Sensor maturity: scans-only costs **+1.68 cars/block on L1** but **+0.02 on L2**. `results/transfer_eval.csv`. *(Equipment vintage still unmodelled — no data axis for it)* |
...
**Score: 6 of 7 fully demonstrated (1, 3, 4, 5, 6, 7). Only Complexity 2 remains paper-only — by decision, with the operator-variation exclusion stated as an ethical choice.**
...
|---|---|---|---|---|---|
| **6** | Scaling: layout / vintage / sensor maturity | ~~28 Aug~~ **DONE 17:10** | — | ✅ **demonstrated** | `scripts/eval_transfer.py` → `results/transfer_eval.csv`. Vintage remains unmodelled and is stated as a gap |
...
|---|---|---|---|
| **08-30 00:50** | Sagar | **The deployment story now exists across the whole submission — and one overclaim was found and made true instead of deleted.** Sagar asked how this would go into a real plant. Checking the code rather than the design docs found **no adapter of any kind** — no OPC-UA, MQTT, Modbus or historian client; the only grep hits were a fault type named `intermittent_connector` and OPC-UA in a comment. Everything reads CSVs. **But the same check found the architecture is genuinely deployment-shaped:** `events.py` is written against *"the five event types a real plant already emits"*, so the integration surface is **seven tables a plant already produces** — boundary scans, PLC state tags, buffer counters, nutrunner results over Open Protocol, andon, rework, calendar. **No new hardware on day one.** The gap is exactly one module, and nothing downstream changes when it lands. **The overclaim:** the proposal's risk table said read-only was *"enforced by a test, not a promise"* — **there was no such test.** That is precisely the class of claim this project has spent two days killing elsewhere, so rather than soften the sentence, **the test was written**: `tests/test_readonly_boundary.py` walks the AST of every module in `src/twin` and `web`, asserting only `store.py`/`sink.py` write, **no module may import a network client** (`socket`, `opcua`, `pymodbus`…), every SQL write names a table the twin owns, and every frame is labelled advisory. **Verified it can fail** by planting a PLC write plus a socket import and confirming both were caught, then restoring cleanly. **Two false positives fixed on the way** — pandas `.rename()` and `str.replace()` were being read as filesystem writes, so filesystem calls are now matched only on `os.`/`shutil.`. Suite goes **10 passed → 43 passed, 8 skipped**. Written into all four artifacts: proposal §2.5 + §9 + risk row, README section, runbook as the **6th prepared judge answer** (it was missing, and it is the most obvious question to ask about a *prototype* round), deck slide 11. **Bonus defect found by measuring the PDF instead of eyeballing it:** `table{page-break-inside:avoid}` made every table atomic, so a long one jumped wholly to the next page and left **page 4 only 31% full**. Tables now break with rows kept intact and headers repeating: fill goes **93/93/55/79/61/64 → 93/95/92/94/94/40**, and the proposal absorbed a whole new section **without gaining a page**. | `5612aba` |
| **08-29 03:30** | Sagar | **Dashboard rebuilt for readability — 16/16 WCAG AA, up from 4/9.** Sagar said the UI wasn't attractive; I presented 5 design directions but **measured the current one first, and it was a defect not a taste question: 5 of 9 colour pairings failed AA** (muted text 3.33:1, **table borders 1.65:1** against a 3.0 requirement) with most failing text at **10px**. **The root mistake: confusing *muted* with *low-contrast*.** ISA-101 asks for restrained **hue**, colour reserved for deviation — it does *not* ask for grey text on grey panels. A plant HMI is read at distance, in bad light, through safety glasses; **legibility is the entire point of the standard** and the old palette had it backwards. Repalletted **against measurements, not by eye**: ground lightened so borders survive on it, panels lifted (ink 16.35:1), muted deepened (7.69:1), borders (4.60:1), signal colours darkened enough to carry white text. The dark leadership card **re-declares its own signal colours** — the light theme's amber would have sat at 1.6:1 there. Also: **10px floor → 12px** throughout including inline JS template styles, tabular figures, **visible keyboard focus (there was none)**, reduced-motion honoured, every colour literal replaced with an audited token. **Layout bug found by looking at it:** the station strip wrapped to two rows at narrower widths — a production line on two rows breaks the only spatial metaphor the screen has, since a supervisor reads left-to-right as flow. Stations now compress instead. **This makes the ISA-101 claim in the proposal true rather than asserted.** | `b1d030f` |
| **08-29 02:20** | Sagar | **SUBMISSION-BLOCKING BUG FOUND BY FRESH-CLONE CHECK: a judge could not have run the prototype.** Cloned the repo as an outsider would and followed `DEMO_RUNBOOK.md` verbatim. The setup command pointed at `../digitaltwin.ai/dataset/...` — **a path that only exists on this machine.** The dataset is gitignored (765 MB), so **anyone cloning this repo — a judge, a reviewer, Priyansh — would have hit "run directory not found" and been unable to start the demo at all.** Fixed by shipping **one sample shift (`sample_data/L1_run_001`, 927 KB)** and making it the server's default, so `python web/server.py --speed 120 --shifts 0` works from a clean clone with **zero arguments and no dataset download**. **Only the observed logs are shipped — no `hidden/` truth files**, because a detector that can read ground truth is not a detector. **Verified by re-cloning from GitHub and starting with no arguments:** 20 stations / 17 instrumented / 3 dark, calibrated confidence live, 17 alerts carrying all five contract fields, all endpoints 200. This is the third time the fresh-clone habit has caught something invisible from the authoring machine — the working-agreement rule earns its place again. | `2b95d0d` |
| **08-29 01:45** | Sagar | **Folded the checklist-honesty and coverage-match findings into both documents — caught two stale claims on the way.** Re-read the proposal against what actually exists rather than assuming it was current, and found **two rows describing work that had since been closed**: Complexity 1 still said "schema built and tested for checklists" (pre-`eval_manual_checks.py`), Complexity 3 still said "window-dated schedule designed" (pre-`sensor_schedule.py`, which had already closed it). Both now state the measured result. **Added a dedicated callout for the reference-parameter match** (75.3% instrumented / 24.7% manual on body+final — exact PS match) that had been buried in a table cell. **Deck's Complexity 1 slide gains a second KPI tile** — 2.76% escape rate alongside the existing 15.5% dark-warning figure — plus the coverage-match callout; rendered via PowerPoint COM, both tiles fit cleanly at 32pt, no overflow. **Proposal now 6 pages** (was 5), rebalanced, no orphan. Tests: 10 passed, 2 skipped (unusually slow this run — 95 min vs the normal ~1 min, background load not a regression, same pass/fail result as every prior run). | `19b55e2` |
| **08-29 01:10** | Sagar | **COMPLEXITY 1 CLOSED — the checklist-honesty test finally run. Coverage now 6 of 7.** Part A §1.3 predicts a manual checklist is confirmation-biased and names the test: compare its pass rate against genealogy-attributed EOL failures. Never run until now. **Across 31,329 entries on 14 stations: pass rate 96.51%, but 835 vehicles it passed went on to fail EOL — a 2.76% escape rate.** A checklist reading near-100% against a non-zero EOL failure rate is measuring *compliance with the checklist*, not quality — exactly as predicted, and structurally the same failure as our `sensor_bias` tools: **the instrument is the thing that is wrong.** Mean **entry latency 4.8 min**, during which the twin has no signal from that station at all — which is why manual data is Tier C *attested*, not a sensor. **Run on L5; the DEMO stays on L1** — this is evidence, not a dataset switch, and switching the demo the day before recording is how demos break. **Two things fixed en route:** (a) the `seg_dark_p` correction was in the code but the **built dataset was stale** at 48.6% dark — code and data disagreed; rebuilt. That correction had also been swept into the proposal commit unmentioned, which was sloppy and is noted. (b) With corrected values the line is 59% instrumented overall, but the number that matters is **body + final assembly at 75.3% instrumented / 24.7% manual** — an *exact* match to the PS's *"majority well-instrumented, a meaningful minority reliant on manual checks"*. Paint is 90% dark **by design**: booth-level sampling is how paint shops work everywhere, a property of the process rather than a coverage gap. | `4dc9f8c` |
...
| **08-28 17:10** | Sagar | **COMPLEXITY 6 CLOSED — layout transfer + sensor maturity, both measured for the first time.** Both experiments existed as code and had **never been run**. **Layout transfer across 4 topologies** (20 stn/1 merge · 30 stn/2 merges · 20 stn + **parallel pair** which breaks the pure-series assumption · 15 stn): **top-1 spans 10.6%–44.5% (a 34-point collapse) while regret spans 1.208–1.431 cars/block (0.22 spread).** The operationally meaningful metric **transfers**; the argmax metric does not — independent evidence for the locked *"regret not top-1"* decision, reached from a different direction than the label-noise argument. Honest bad news: capture against the per-layout ceiling falls from **71.7% (L1) to 23–42%**, though the ceilings differ sharply (4.27 vs 1.71), so those layouts have far less available gain. **Sensor maturity** (whole pipeline from boundary scans, **zero PLC state tags**) splits informatively: **L1 regret 1.208 → 2.891 (+1.683)** but **L2 1.218 → 1.239 (+0.021)** — state tags matter enormously where a strong constraint exists and almost not at all on a balanced line. **Sensor maturity matters most exactly where the opportunity is largest**, which is a far better input to a retrofit business case than one average. **Caveat kept with the numbers:** L2–L4 are 12 runs / ~95 blocks each, intervals are wide (L2 top-1 [5.9, 18.5]); the dataset imbalance Priyansh flagged is real, so these are directional. | `23e3559` |
